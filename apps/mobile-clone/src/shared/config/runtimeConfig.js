...
const API_BASE_URL_KEY = "brawlgg_mobile_clone_api_base_url";
const FALLBACK_API_BASE_URL = "https://brawlgg-server.dsmhs.kr";
const LEGACY_LOCAL_DEFAULTS = new Set(["http://127.0.0.1:3000", "http://localhost:3000"]);
...
export async function hydrateRuntimeConfig() {
  ...
  try {
    ...
    if (storedBaseUrl) {
      ...
      runtimeApiBaseUrl = normalizedStoredBaseUrl;

      if (LEGACY_LOCAL_DEFAULTS.has(normalizedStoredBaseUrl)) {
        runtimeApiBaseUrl = normalizeApiBaseUrl(readDefaultApiBaseUrl());
        await AsyncStorage.setItem(API_BASE_URL_KEY, runtimeApiBaseUrl);
      }
    }
  } catch {
    runtimeApiBaseUrl = normalizeApiBaseUrl(readDefaultApiBaseUrl());
